...
class AnyRORScraper:
    ...
    def select_district_by_name(self, name: str):
        """Select district by name OR value (partial match)"""
        options = self.get_options(self.SELECTORS["district"])
        if not options:
            return None
        # Try exact value match first (e.g. "07"), then text match
        selected = next((o for o in options if o["value"] == name), None)
        if not selected:
    ...
    def select_taluka_by_name(self, name: str):
        """Select taluka by name OR value (partial match)"""
        options = self.get_options(self.SELECTORS["taluka"])
        if not options:
            return None
    ...
    def select_village_by_name(self, name: str):
        """Select village by name OR value (partial match)"""
    ...
    def select_survey_by_value(self, value: str):
        ...
        # Wait for survey dropdown to populate after village selection
        time.sleep(2)
        options = self.get_options(self.SELECTORS["survey_no"])
        if not options:
            return None
    ...
    def extract_data(self) -> dict:
        ...
        # VF-7 uses <pre> tags for ownership/encumbrance sections, not <table>
        ...
        # Also extract <pre> tags - VF-7 ownership details are in pre blocks
        pre_blocks = self.page.locator("pre").all()
        for i, pre in enumerate(pre_blocks):
            text = pre.text_content()
            if text and len(text.strip()) > 50:
                data["tables"].append({"index": f"pre_{i}", "text": text.strip()})
    ...
    def scrape_by_owner(self, district_code: str, taluka_code: str, village_code: str, owner_name: str, max_captcha_attempts: int = 3):
        """
        Search for land records by owner name.
        Uses record type 13: KNOW KHATA BY OWNER NAME
        Returns list of matching records (khata_no, survey_no, owner_name)
        """
        self.navigate()

        # Select "KNOW KHATA BY OWNER NAME" (option 10)
        print(f"[INFO] Selecting 'Know Khata by Owner Name' search...")
        self.page.locator(self.SELECTORS["record_type"]).select_option("10")
        self.wait_for_page()

        # Select location
        print(f"[INFO] Setting location: district={district_code}, taluka={taluka_code}, village={village_code}")

        d = self.select_district_by_name(district_code)
        if not d:
            return {"success": False, "error": f"District {district_code} not found", "results": []}

        t = self.select_taluka_by_name(taluka_code)
        if not t:
            return {"success": False, "error": f"Taluka {taluka_code} not found", "results": []}

        v = self.select_village_by_name(village_code)
        if not v:
            return {"success": False, "error": f"Village {village_code} not found", "results": []}

        # Enter owner name - wait for input field to appear
        print(f"[INFO] Entering owner name: {owner_name}")
        time.sleep(1)  # Wait for form to fully load after village selection

        owner_input = self.page.locator(self.SELECTORS["owner_name_input"])
        if not owner_input.count() or not owner_input.is_visible():
            # Try alternate selectors with correct casing
            alt_selectors = [
                "#ContentPlaceHolder1_txtownername",
                "input[id*='ownername']",
                "input[id*='txtowner']",
            ...
            for sel in alt_selectors:
                try:
                    ...
                    if candidate.count() > 0 and candidate.is_visible():
                        # Make sure it's not the captcha input
                        placeholder = candidate.get_attribute("placeholder") or ""
                        if "captcha" not in placeholder.lower() and "text shown" not in placeholder.lower():
                            owner_input = candidate
                            print(f"[INFO] Found owner input via: {sel}")
                            break
                except:
                    continue

        if owner_input.count() and owner_input.is_visible():
            owner_input.fill(owner_name)
            time.sleep(0.5)
        else:
            # Screenshot for debugging
            self.page.screenshot(path="output/owner_input_not_found.png", full_page=True)
            return {"success": False, "error": "Owner name input field not found", "results": []}
        ...
        for attempt in range(1, max_captcha_attempts + 1):
            ...
            results = self._extract_owner_search_results()

            if results:
                print(f"[SUCCESS] Found {len(results)} matches for '{owner_name}'")
                return {"success": True, "results": results, "district": d, "taluka": t, "village": v}

            # Check for "no records" message
            page_text = self.page.text_content("body") or ""
            if "કોઈ રેકોર્ડ નથી" in page_text or "No Record" in page_text.lower() or "not found" in page_text.lower():
                print(f"[INFO] No records found for '{owner_name}' in this village")
                return {"success": True, "results": [], "message": "No records found"}
        ...
        return {"success": False, "error": "Max captcha attempts reached", "results": []}

    def _extract_owner_search_results(self) -> list:
        """Extract results from owner name search results table"""
        results = []

        # Look for results table - usually contains ક્રમ (Sr), ખાતા નંબર (Khata), સરવે નંબર (Survey), ખાતેદારનું નામ (Owner)
        tables = self.page.locator("table").all()

        for table in tables:
            text = table.text_content() or ""
            # Check if this is the results table
            if "ખાતા નંબર" in text or "સરવે" in text or "ખાતેદાર" in text:
                ...
                for row in rows:
                    ...
                    if len(cells) >= 4:
                        try:
                            ...
                            owner = cells[3].text_content().strip()

                            # Skip header rows
                            if "ક્રમ" in sr_no or "ખાતા" in khata_no:
                                continue

                            if khata_no and survey_no:
                                results.append({
                                    "sr_no": sr_no,
                                    "khata_no": khata_no,
                                    "survey_no": survey_no,
